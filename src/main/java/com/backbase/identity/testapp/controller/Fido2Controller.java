...
public class Fido2Controller {
    ...
    private static final Logger log = LoggerFactory.getLogger(Fido2Controller.class);
    ...
    public PublicKeyAuthenticationRequest handleAssertion(@RequestBody AssertionEnvelope envelope) {
        // Simulate platform authenticator logic here
        PublicKeyAuthenticationRequestBuilder builder =
            new PublicKeyAuthenticationRequestBuilder(fido2KeyPairs, envelope.getOptions());

        CredentialAssertionOptions options = envelope.getOptions();
        if (StringUtils.isNotBlank(envelope.getCredentialId())) {
            log.debug("Using credential ID from envelope: {}", envelope.getCredentialId());
            ...
            log.debug("No credential ID provided in envelope");
            if (options.getAllowCredentials() != null && !options.getAllowCredentials().isEmpty()) {
                log.debug("Using first credential ID from allowed list");
                builder.withCredentialId(envelope.getOptions().getAllowCredentials().getFirst().getId());
            } else {
                log.debug("No credential ID in allowed list");
                throw new IllegalArgumentException("No allowed credentials available to use for authentication.");
            }
        }

        if (options.getExtensions() != null && options.getExtensions().getExtensions() != null
            && options.getExtensions().getExtensions().containsKey("rar")) {
            log.debug("rar found in extensions, adding to client data");
            builder.withClientDataEntry("rar", options.getExtensions().getExtensions().get("rar"));
        }
